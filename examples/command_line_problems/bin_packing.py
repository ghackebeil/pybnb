...
class BinPacking(pybnb.Problem):
    def __init__(
        self, V, W, pyomo_solver="ipopt", pyomo_solver_io="nl", integer_tolerance=1e-4
    ):
        assert V > 0
        ...
        for j in N:
            m.assign_1[j] = pmo.constraint(sum(x[i, j] for i in N) == 1)
        ...
        self.opt = pmo.SolverFactory(pyomo_solver, solver_io=pyomo_solver_io)
    ...
    def _branch_y(self):
        ...
        orig_bounds.update((yi, yi.bounds) for yi in self.model.y.components())
        orig_bounds.update((xij, xij.bounds) for xij in self.model.x.components())

        children = [pybnb.Node(), pybnb.Node()]
    ...
    def _branch_x(self):
        ...
        for j in unassigned_items:
            for i in N:
                if (self.model.x[i, j].lb == 0) and (self.model.x[i, j].ub == 1):
                    bv = self.model.x[i, j]
                    break
            if bv is not None:
                break
    ...
    def objective(self):
        ...
        item_order = []
        for j in N:
            assigned = sum(bool(self.model.x[i, j].value == 1) for i in N)
            if assigned == 1:
                for i in N:
        ...
        bin_weight = {
            i: sum(self.W[j] * self.model.x[i, j].value for j in N) for i in N
        }
    ...
    def bound(self):
        ...
        if (str(results.solver.status) == "ok") and (
            ...
            if objective - math.floor(objective) > self._integer_tolerance:
            ...
            assert "Restoration Phase Failed" in results.solver.message
            # assume infeasible
            return self.infeasible_objective()
