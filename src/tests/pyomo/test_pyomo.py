...
@pytest.mark.skipif(not pyomo_available, reason="Pyomo is not available")
class Test(object):
    def test_correct_integer_lb(self):
        ...
        for b in [-4, -3, -2, -1, 0, 1, 2, 3, 4]:
            assert correct_integer_lb(b - tol - eps, integer_tolerance=tol) == b
            assert correct_integer_lb(b - tol, integer_tolerance=tol) == b
            assert correct_integer_lb(b - tol + eps, integer_tolerance=tol) == b
            assert correct_integer_lb(b - eps, integer_tolerance=tol) == b
            assert correct_integer_lb(b, integer_tolerance=tol) == b
            assert correct_integer_lb(b + eps, integer_tolerance=tol) == b
            assert correct_integer_lb(b + tol - eps, integer_tolerance=tol) == b
            assert correct_integer_lb(b + tol, integer_tolerance=tol) == b
            assert correct_integer_lb(b + tol + eps, integer_tolerance=tol) == b + 1
            assert correct_integer_lb(b + 1 - eps, integer_tolerance=tol) == b + 1
            assert correct_integer_lb(b + 1, integer_tolerance=tol) == b + 1
            assert correct_integer_lb(b + 1 + eps, integer_tolerance=tol) == b + 1
            assert correct_integer_lb(b + 1 + tol - eps, integer_tolerance=tol) == b + 1
            assert correct_integer_lb(b + 1 + tol, integer_tolerance=tol) == b + 1
            assert correct_integer_lb(b + 1 + tol + eps, integer_tolerance=tol) == b + 2
    ...
    def test_correct_integer_ub(self):
        ...
        tol = 0.25
        for b in [-4, -3, -2, -1, 0, 1, 2, 3, 4]:
            assert correct_integer_ub(b - tol - eps, integer_tolerance=tol) == b - 1
            assert correct_integer_ub(b - tol, integer_tolerance=tol) == b
            assert correct_integer_ub(b - tol + eps, integer_tolerance=tol) == b
            assert correct_integer_ub(b - eps, integer_tolerance=tol) == b
            assert correct_integer_ub(b, integer_tolerance=tol) == b
            assert correct_integer_ub(b + eps, integer_tolerance=tol) == b
            assert correct_integer_ub(b + tol - eps, integer_tolerance=tol) == b
            assert correct_integer_ub(b + tol, integer_tolerance=tol) == b
            assert correct_integer_ub(b + tol + eps, integer_tolerance=tol) == b
            assert correct_integer_ub(b + 1 - eps, integer_tolerance=tol) == b + 1
            assert correct_integer_ub(b + 1, integer_tolerance=tol) == b + 1
            assert correct_integer_ub(b + 1 + eps, integer_tolerance=tol) == b + 1
            assert correct_integer_ub(b + 1 + tol - eps, integer_tolerance=tol) == b + 1
            assert correct_integer_ub(b + 1 + tol, integer_tolerance=tol) == b + 1
            assert correct_integer_ub(b + 1 + tol + eps, integer_tolerance=tol) == b + 1
    ...
    def test_sense(self):
        class Max(PyomoProblem):
            def __init__(self):
                ...
                self._pyomo_model.o = pmo.objective(sense=pmo.maximize)
