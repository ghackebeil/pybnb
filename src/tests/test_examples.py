...
if pyomo_available:  # pragma:nocover
    ...
    if ipopt is None or isinstance(ipopt, UnknownSolver):
        ...
        ipopt_available = (ipopt.available(exception_flag=False)) and (
            (not hasattr(ipopt, "executable")) or (ipopt.executable() is not None)
        )
...
examples.append(os.path.join(exdir, "scripts", "tsp", "tsp_byvertex.py"))
examples.append(os.path.join(exdir, "scripts", "tsp", "tsp_byedge.py"))
...
for fname in examples:
    ...
    basename = basename[:-3]
    if basename in ("tsp_byvertex", "tsp_byedge"):
        for datafile in ("p01_d", "p01_d_inf"):
            tname = "test_" + basename + "_" + datafile
            bname = os.path.join(baselinedir, basename + "_" + datafile + ".yaml")
            tdict[tname] = (
                fname,
                bname,
                [os.path.join(exdir, "scripts", "tsp", datafile + ".txt")],
            )
    else:
        tname = "test_" + basename
        bname = os.path.join(baselinedir, basename + ".yaml")
        args = None
        if basename in ("rosenbrock_2d", "lipschitz_1d"):
...
tdict["test_binary_knapsack_nested"] = (
    ...
    ["--nested-solver", "--nested-node-limit=100", "--nested-queue-strategy=random"],
)
...
@pytest.mark.parametrize(("example_name", "procs"), scenarios)
@pytest.mark.example
def test_example(example_name, procs):
    ...
    if (example_name == "test_simple") or ("tsp_by" in example_name):
    ...
    try:
        if procs == 1:
            if ("range_reduction_pyomo" in example_name) or ("tsp_by" in example_name):
                rc = subprocess.call(
                    cmd + ["--results-file", results_filename] + options
                )
                ...
                rc = subprocess.call(
                    cmd
                    + ["--disable-mpi", "--results-file", results_filename]
                    + options
                )
        else:
            assert procs > 1
            if subprocess.call(
                ["mpirun", "--allow-run-as-root", "--version"],
        ...
        for key in baseline_results:
            if type(baseline_results[key]) is float:
                assert round(baseline_results[key], 4) == round(results[key], 4)
            else:
