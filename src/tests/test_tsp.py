import sys
import os
import random
import math

thisdir = os.path.dirname(os.path.abspath(__file__))
exdir = os.path.join(
    os.path.dirname(os.path.dirname(thisdir)),
    "examples")
tspdir = os.path.join(exdir,"scripts","tsp")

sys.path.insert(0,tspdir)
try:
    from tsp_naive import run_2opt
finally:
    sys.path.remove(tspdir)

class TestHeuristics(object):

    def _generate_random_matrix(self, size, symmetric=False):
        assert size > 1
        dist = [[None for j in range(size)]
               for i in range(size)]
        for i in range(size):
            dist[i][i] = 0
            for j in range(i):
                assert i != j
                dist[i][j] = random.randint(1,1000)
                if symmetric:
                    dist[j][i] = dist[i][j]
                else:
                    dist[j][i] = random.randint(1,1000)
        return dist

    def _compute_tour_cost(self, dist, tour):
        N = len(tour)
        assert N == len(dist)+1
        assert tour[0] == tour[-1]
        cost = 0
        for k in range(N-1):
            assert tour[k] != tour[k+1]
            assert dist[tour[k]][tour[k+1]] != 0
            cost += dist[tour[k]][tour[k+1]]
        assert not math.isinf(cost)
        assert not math.isnan(cost)
        return cost

    def test_run_2opt(self):
        for n in range(1,7):
            size = 2**n
            for dist in (self._generate_random_matrix(size, symmetric=True),
                         self._generate_random_matrix(size, symmetric=False)):
                tour = list(range(size))
                tour.append(tour[0])
                cost = self._compute_tour_cost(dist, tour)
                new_cost, new_tour = run_2opt(dist, cost, tour)
                if new_tour != tour:
                    assert new_cost < cost
                new_cost_ = self._compute_tour_cost(dist, new_tour)
                assert new_cost_ == new_cost
