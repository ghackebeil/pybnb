...
exdir = os.path.join(os.path.dirname(os.path.dirname(thisdir)), "examples")
...
class TestHeuristics(object):
    def _generate_random_matrix(self, size, symmetric=False):
        assert size > 1
        dist = [[None for j in range(size)] for i in range(size)]
        for i in range(size):
            dist[i][i] = 0
            for j in range(i):
                assert i != j
    ...
    def test_run_2opt(self):
        for n in range(1, 7):
            size = 2 ** n
            for dist in (
                self._generate_random_matrix(size, symmetric=True),
