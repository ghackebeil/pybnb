...
class TestConvergenceChecker(object):
    ...
    def test_check_termination_criteria(self):
        ...
        p = ConvergenceChecker(minimize)
        assert (
            p.check_termination_criteria(inf, None) is TerminationCondition.optimality
        )
        assert p.check_termination_criteria(0, 0) is TerminationCondition.optimality
        ...
        assert (
            p.check_termination_criteria(0, 1) is TerminationCondition.objective_limit
        )
        p = ConvergenceChecker(minimize, objective_stop=inf)
        assert p.check_termination_criteria(-inf, inf) is None
        assert (
            p.check_termination_criteria(-inf, 1000000)
            is TerminationCondition.objective_limit
        )
        ...
        assert p.check_termination_criteria(-inf, inf) is None
        assert (
            p.check_termination_criteria(-10000000, inf)
            is TerminationCondition.objective_limit
        )
        # max
        p = ConvergenceChecker(maximize)
    ...
    def test_objective_is_optimal(self):
        ...
        for sense in [minimize, maximize]:
            ...
            for bound, objective in itertools.product(
    ...
    def test_compute_relative_gap(self):
        ...
        for sense in [minimize, maximize]:
            ...
            for bound, objective in itertools.product(
                ...
                if (not math.isinf(bound)) or (not math.isinf(objective)):
                    continue
                if bound == objective:
                    assert p.compute_relative_gap(bound, objective) == 0
                elif p.sense == minimize:
                    if (bound == -inf) or (objective == inf):
                        assert p.compute_relative_gap(bound, objective) == inf
                    else:
                        assert p.compute_relative_gap(bound, objective) == -inf
                else:
                    assert p.sense == maximize
